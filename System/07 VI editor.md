...
| ---- | ------------------------------------------------------------ |
| r    | 1글자 변경                                                   |
| R    | 현재 커서 위치로 부터 수정 (잠깐 편집모드로 전환된다. esc로 다시 명령 모드로) |
| cw   | 1단어 변경                                                   |
| cc   | 1줄 변경, 현재 행을 모두 지운 후 입력 모드                   |

### 일관 텍스트 변경

| 예제            | 의미                             |
| --------------- | -------------------------------- |
| :s/aaaa/bbbb    | 현재행의 aaaa를 bbbb로 변경      |
| :s/aa/bb/g      | 현재행의 aa를 bb로 모두 변경     |
| :1,$s/00/11/g   | 전체 파일에서 00을 11로 변경     |
| :26,32s/ls/sl/g | 26행부터 32행까지 ls를 sl로 변경 |

### 탐색 모드

![image](https://user-images.githubusercontent.com/68107000/97517839-55063580-19d9-11eb-8515-df20f8299e4a.png)

| 명령     | 의미                                                   |
| -------- | ------------------------------------------------------ |
| /pattern | 현재 위치에서 정방향으로 탐색                          |
| ?pattern | 역방향으로 탐색                                        |
| n        | 이전 탐색 방향으로 탐색 (/일 땐 정방향, ?일 땐 역방향) |
| N        | 이전 역방향으로 탐색                                   |
...
| ------ | ------------------------------------------------------------ |
| u      | Undo, 변경한 내용 취소                                       |
| U      | 최종 변경 작업을 수행했던 행에서 변경된 내용을 모두 원상 복구 |
| Ctrl+R | Redo, 취소한 내용을 다시 실행                                |

### Repeat Last Change

| 명령 | 의미                                             |
| ---- | ------------------------------------------------ |
| .    | 직전 변경한 작업을 현재  커서 위치에서 반복 수행 |

### 기타 기능

| 명령   | 의미                                                         |
| ------ | ------------------------------------------------------------ |
| Ctrl+g | 요약줄에 현재 편집 상태 표시<br />파일 이름, 파일의 줄수, 커서는 몇 퍼센트에 위치하는지 알려줌 |
| J      | 현재행과 아래행을 합침                                       |
...
| 유용한 set 옵션                                | 의미                               |
| ---------------------------------------------- | ---------------------------------- |
| :set background=dark\|light                    | 어둡거나 밝은 화면을 기준으로 배색 |
| :set [no]number<br />(줄임말) :se nu, :se nonu | 행 앞에 라인넘버 표시              |
| :set [no]cursorline                            | 커서가 위치한 행을 표시            |
| :set [no]cursorcolumn                          | 커서가 위치한 컬럼을 표시          |
